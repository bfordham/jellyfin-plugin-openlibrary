...
/// <summary>
/// HTTP handler that implements client-side rate limiting for OpenLibrary API requests.
/// </summary>
internal sealed class ClientSideRateLimitedHandler : DelegatingHandler
{
    ...
    {
        using RateLimitLease lease = await _rateLimiter.AcquireAsync(permitCount: 1, cancellationToken: cancellationToken).ConfigureAwait(false);
        ...
        if (lease.TryGetMetadata(MetadataName.RetryAfter, out TimeSpan retryAfter))
        {
            ...
            response.Headers.Add(HeaderNames.RetryAfter, retryAfter.TotalSeconds.ToString("F0", NumberFormatInfo.InvariantInfo));
        }
